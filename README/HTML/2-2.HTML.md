...
- 먼저 `http://localhost:8080/`에 접속했을때 Hello World가 나타나는 웹사이트를 만들어 보도록 하죠
...
- `<body> </body>` 바디 태그안에 내용물을 넣어서 페이지를 만들어 보고 `tag`를 넣어 실습을 진행핡것입니다.

- 하지만 너무 많은 태그는 너무나도 많고 사용되지 않는 태그들도 여러개 있기 때문에 `Tag` 랭킹에서 가장 많이 사용되는 태그를 중점으로 이야기하겠습니다.
